...
bot.onText(/\/join/, (msg) => {
  utils.userExists(msg.from.id).then((exists) => {
    if (exists)
      bot.sendMessage(
        msg.chat.id,
        "You're already playing. Try listing with /list_judasmen"
      );
    else {
      bot
        .sendMessage(
          msg.chat.id,
          "This is a private bot. Please enter a password to access.",
          force_reply
        )
        .then((payload) => {
          const replyListenerId = bot.onReplyToMessage(
            ...
            (msg) => {
              ...
              if (passResponse == process.env.PASS) {
                ...
                bot.sendMessage(
                  msg.chat.id,
                  "Welcome to the game " + msg.from.first_name + "!"
                );
              } else {
...
bot.onText(/\/change_date_to/, (msg) => {
  bot
    .sendMessage(
      msg.chat.id,
      "Please **mention reply** with your last judas date (yyyy/mm/dd)",
      force_reply
    )
    .then((payload) => {
      const replyListenerId = bot.onReplyToMessage(
        ...
        (msg) => {
          ...
          if (!dateIsValid) {
            bot.sendMessage(
              msg.chat.id,
              "Date is invalid. Correct format is yyyy/mm/dd"
            );
            return;
          }
          ...
          utils.userExists(msg.from.id).then((exists) => {
            if (exists) {
              ...
              bot.sendMessage(
                msg.chat.id,
                "Date of " + msg.from.first_name + " has been updated."
              );
            } else {
              bot.sendMessage(
                msg.chat.id,
                msg.from.first_name +
                  ", you're not playing. If you want to join, send /join."
              );
            }
          });
        }
...
bot.onText(/\/set_birthdate/, (msg) => {
  bot
    .sendMessage(
      msg.chat.id,
      "Please **mention reply** with your birthdate (yyyy/mm/dd)",
      force_reply
    )
    .then((payload) => {
      const replyListenerId = bot.onReplyToMessage(
        ...
        (msg) => {
          ...
          utils
            .setBirthDate(msg.from.id, birthDate.getTime())
            .then((dateIsSet) => {
              if (!dateIsSet) {
                bot.sendMessage(
                  msg.chat.id,
                  "You're not playing. If you'd like to start, please /join"
                );
                return;
              }
...
function registerPartner(msg, type) {
  bot
    .sendMessage(
      msg.chat.id,
      "Please **mention reply** with the name of the partner",
      force_reply
    )
    .then((payload) => {
      const replyListenerId = bot.onReplyToMessage(
  ...
  utils.userExists(msg.from.id).then((exists) => {
    ...
    utils.freePartner(msg.from.id).then((freed) => {
      if (freed)
        bot.sendMessage(
          msg.chat.id,
          msg.from.first_name + " has been freed. Run. RUN!"
        );
      else
        bot.sendMessage(
          msg.chat.id,
          "Whatchu running from? You ain't got no one."
        );
    });
  });
});
